...
export const musicPlaylistSchema = z.object({
  ...
  trackCaptions: z.array(z.array(z.any())).default([]).describe('Per-track captions (filled by calculateMetadata)'),
});
...
> = async ({ props }) => {
  ...
  // Calculate duration and captions for each audio file in parallel
  const [audioDurationsInSeconds, trackCaptions] = await Promise.all([
    Promise.all(audioFiles.map((src) => getAudioDuration(src).catch(() => 0))),
    Promise.all(
      audioFiles.map(async (src): Promise<Caption[]> => {
        const captionPath = getCaptionFileForAudio(src, audioDir);
        if (!captionPath) return [];
        try {
          const res = await fetch(captionPath + `?t=${Date.now()}`);
          if (!res.ok) return [];
          return normalizeCaptions(await res.json());
        } catch {
          return [];
        }
      }),
